...
function onRequest(request, response) {
  ...
  function sendResponse() {
    var date = new Date(); // シード値用の日付データ

    var query = querystring.parse(request.data);
    
    var result = createfortune(date, query); // 今日の運勢生成
    
    date.setDate(date.getDate() + 1); // 日付の変更
    var result2 = createfortune(date, query); // 明日の運勢生成
...
//　明日の運勢も表示させる場合、２回書くのはめんどくさいので関数化
function createfortune(date, query){
